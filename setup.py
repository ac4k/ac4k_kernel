...
def detect_cuda_arch() -> str:
    ...
    raise RuntimeError(
        "[ac4k] Could not auto-detect CUDA architecture.\n"
...
def get_cuda_extension(arch: str):
    ...
    if arch not in NVCC_ARCH_FLAGS:
        raise ValueError(f"No nvcc arch flag defined for {arch}. Add it to NVCC_ARCH_FLAGS.")
    ...
    print(f"[ac4k] Parallel jobs: {MAX_PARALLEL_JOBS}, nvcc threads: {nvcc_threads}")
    ...
    return CUDAExtension(
        ...
        extra_compile_args={
            ...
            'nvcc': [
                ...
                '-Xcompiler', '-pipe',                # Host compiler uses pipes
            ]
        },
        extra_link_args=[
