"""
Sparse linear attention operator for ac4k_kernel.
"""
...
def mean_pool(x, WINDOW_SIZE):
    ...
    B, H, L, D = x.shape
    L_BLOCKS = ceil_div(L, WINDOW_SIZE)
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        _triton_attn_fwd[grid](q,
                               ...
                               BLOCK_Q=BLOCK_Q,
                               BLOCK_KV=BLOCK_KV,
                               num_warps=align_up(ceil_div(BLOCK_Q, 16), 4),
                               num_stages=3)
    ...
    @staticmethod
    def backward(ctx, do):
        q, k, v, sparse_mask, lut, lse, o = ctx.saved_tensors
        do = do.contiguous()
