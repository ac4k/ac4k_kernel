...
@lru_cache(maxsize=1)
def _load_cuda_qk_nvfp4_pv_f16_mha():
    try:
...
def _qk_nvfp4_pv_f16_attention(q, k, v, layout="BNHD", out=None):
    assert layout in ["BNHD", "BHND"], "Unsupported layout: {}".format(layout)

    kernel = _load_cuda_qk_nvfp4_pv_f16_mha()
...
def attention(q, k, v, layout="BNHD", precision="nvfp4", out=None):
    ...
    assert precision in [
        "nvfp4", "nvfp4+fp8e4m3"
    ], "only support nvfp4 and nvfp4+fp8e4m3, but got {}".format(precision)

    if precision == "nvfp4":
        return _nvfp4_attention(q, k, v, layout, out)
    else:
        return _qk_nvfp4_pv_f16_attention(q, k, v, layout, out)
