...
static __device__ __forceinline__ int8_t float_to_int8(float in) {
  uint32_t val;
  asm volatile("cvt.rni.sat.s8.f32 %0, %1;" : "=r"(val) : "f"(in));
  return reinterpret_cast<const int8_t &>(val);
}

__global__ void int8_quantize_sm120_kernel(
    INT8 *out, float *sf, const BF16 *in, const int32_t *scale_max,
  ...
  /// OUT stride
  /// [xx, xx, cross_dim, reduce_dim_align] x INT8
...
#pragma unroll
      for (int j = 0; j < TILE_THREAD_REDUCE_DIM; ++j) {
        auto i8 = float_to_int8(in_f32[j]);
...
/// OUT layout
/// [xx, xx, cross_dim_align, reduce_dim_align] x INT8
///
/// SF layout
/// [xx, xx, cross_dim_align] x F32
void int8_quantize_sm120(torch::Tensor &out, torch::Tensor &sf,
  ...
  /// Check scale_max
  CHECK_INPUT(scale_max, at::ScalarType::Int, "scale_max must be int32");
  ...
  CHECK_OUTPUT(out, at::ScalarType::Char, "out must be pack to int8 tensor");
  ...
  int8_quantize_sm120_kernel<<<grid, block, 0, stream>>>(
      reinterpret_cast<INT8 *>(out.data_ptr()),
      reinterpret_cast<float *>(sf.data_ptr()),
      reinterpret_cast<const BF16 *>(in.data_ptr()),
      reinterpret_cast<const int32_t *>(scale_max.data_ptr()), out_shape[0],
