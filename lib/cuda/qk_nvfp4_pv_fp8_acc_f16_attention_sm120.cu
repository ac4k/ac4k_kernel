...
constexpr int TILE_V_BLOCK_SIZE = TILE_V_BLOCK_ELES * sizeof(F8E4M3);
...
__launch_bounds__(CONSUMER_THREAD_NUM + PRODUCER_THREAD_NUM, 1) __global__
    void qk_nvfp4_pv_fp8_acc_fp16_mha_fwd_sm120_kernel(
        BF16 *O, int64_t o_b_stride, int64_t o_h_stride, int64_t o_n_stride,
        int64_t o_d_stride, const float *q_global_scale,
        const float *k_global_scale, int64_t B, int64_t H, int64_t Nq,
        int64_t Nkv, int64_t Dqk, int64_t Dv, float qk_norm,
...
#pragma unroll
    ...
    /// Q
    AFrag_NVFP4_16x64 q_frag[TILE_DOT0_WARP_M / TILE_DOT0_ATOMIC_M]
...
#pragma unroll
          for (int j = 0; j < TILE_DOT0_WARP_K / TILE_DOT0_ATOMIC_K; ++j) {
            ...
            int q_shared_col = (j * TILE_DOT0_ATOMIC_K + (lane_id / 16) * 32) /
                               ELES_PER_NVFP4x2;
            ...
            if (lane_id % 4 < 2) {
              ...
              int m = warp_dot0_m + i * TILE_DOT0_ATOMIC_M + (lane_id % 2) * 8 +
                      lane_id / 4;
              // k map to quantize
              int k = j * TILE_DOT0_ATOMIC_K;
...
#pragma unroll
      ...
      ///         8 * 4 = 32 cycles
      float qk_scale = (*q_global_scale) * (*k_global_scale);
#pragma unroll
      for (int i = 0; i < TILE_DOT0_WARP_M / TILE_DOT0_ATOMIC_M; ++i) {
#pragma unroll
...
#pragma unroll
          for (int k = 0; k < s_frag[i][j].REGISTERS_PER_THREAD; ++k) {
            s_frag[i][j].data[k] = s_frag[i][j].data[k] * qk_scale;
          }
        }
      }
...
#pragma unroll
      ...
      //       /// Step 1: S = S * alpha0(nvfp4 global scale) *
      //       qk_norm(rsqrt(dk))
      //       ///         8 * 4 = 32 cycles
      //       float qk_scale = (*q_global_scale) * (*k_global_scale) * qk_norm;
      //       float softmax_scale = qk_scale * 1.4426950408889634f;
      // #pragma unroll
      //       for (int i = 0; i < TILE_DOT0_WARP_M / TILE_DOT0_ATOMIC_M; ++i) {
      // #pragma unroll
      //         for (int j = 0; j < TILE_DOT0_WARP_N / TILE_DOT0_ATOMIC_N; ++j)
      ...
      // #pragma unroll
      //           for (int k = 0; k < s_frag[i][j].REGISTERS_PER_THREAD; ++k) {
      //             s_frag[i][j].data[k] = s_frag[i][j].data[k] * softmax_scale
      //             - S_FP8_OFFSET;
      //           }
      //         }
      //       }
      ...
      //         } // end loop j
      //         max_new0[i] =
      //             fmaxf(__shfl_xor_sync(0xffffffff, max_new0[i], 1),
      //             max_new0[i]);
      //         max_new0[i] =
      //             fmaxf(__shfl_xor_sync(0xffffffff, max_new0[i], 2),
      //             max_new0[i]);
      //         max_new1[i] =
      //             fmaxf(__shfl_xor_sync(0xffffffff, max_new1[i], 1),
      //             max_new1[i]);
      //         max_new1[i] =
      //             fmaxf(__shfl_xor_sync(0xffffffff, max_new1[i], 2),
      //             max_new1[i]);
      //       } // end loop i
...
#pragma unroll
      ...
      //   printf("p_frag[0]0][0]=%f, p_frag[0]0][1]=%f, p_frag[0]0][2]=%f,
      //   p_frag[0]0][3]=%f\n",
      //         p_frag[0][0].data[0], p_frag[0][0].data[1],
      //         p_frag[0][0].data[2], p_frag[0][0].data[3]);
      // }
...
//===--------------------------------------------------------------------===//
// ac4k mha attention with nvfp4 & fp8 acceleration
// Q               :[B,   H,    Nq,     Dqk/2]xNVFP4
// K               :[B,   H,    Nkv,    Dqk/2]xNVFP4
// V               :[B,   H,    Dv,     Nkv]xFP8E4M3
// O               :[B,   H,    Nq,     Dv]xBF16
// Q_SF            :[B,   H,    Dqk/64, Nq,   4]xF8E4M3
// K_SF            :[B,   H,    Dqk/64, Nkv,  4]xF8E4M3
// V_SF            :[B,   H,    Dv]xF32
// Q_GLOBAL_SCALE  :F32
// K_GLOBAL_SCALE  :F32
...
void qk_nvfp4_pv_fp8_acc_fp16_mha_fwd_sm120(
  ...
  CUtensorMap k_tensor_map = create_4d_tensor_map<
      ...
      CUtensorMapDataType::CU_TENSOR_MAP_DATA_TYPE_UINT8, NVFP4x2, SwizzleK>(
  ...
  kernel<<<grid, block, smem_size, stream>>>(
      ...
      reinterpret_cast<const float *>(k_global_scale.data_ptr()), B, H, Nq, Nkv,
